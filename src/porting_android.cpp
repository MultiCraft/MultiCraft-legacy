...
float getMemoryMax()
{
	if (device_memory_max == 0) {
		...
		FATAL_ERROR_IF(getMemory == nullptr,
			"porting::getMemoryMax unable to find java getMemoryMax method");
...
bool upgrade(const std::string &item)
{
	...
	return jnienv->CallBooleanMethod(app_global->activity->clazz, upgradeGame, jitem);
}
